...
- Python 构建环境改为探测可执行的 3.11 解释器，失效虚拟环境可自动重建
- Java 构建环境严格探测 JDK 17，后端与桌宠构建支持重复执行
- Electron 开发态优先使用项目 Python 环境
- 本地接口认证令牌改为 Windows 加密持久化，健康检查纳入认证
- RAG 启动仅在索引异常或迁移时重建 FTS，向量分块增加内存缓存与 Top-K 检索
- 清理废弃 Electron 桌宠入口、无调用 IPC 和 Java 渲染残留

### 当前边界

- GPT-SoVITS 第三方源码已准备，但模型权重、训练产物和应用接线尚未完成
- 发布安装包仍需代码签名证书
