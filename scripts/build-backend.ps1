...
$output = Join-Path $backend 'dist-onefile-v10\kaltsit-backend.exe'
...
& $python -m PyInstaller `
    --distpath (Join-Path $backend 'dist-onefile-v10') `
    --workpath (Join-Path $backend 'build-pyinstaller-onefile-v10') `
