...
$output = Join-Path $backend 'dist-onefile-v8\kaltsit-backend.exe'
...
& $python -m PyInstaller `
    --distpath (Join-Path $backend 'dist-onefile-v8') `
    --workpath (Join-Path $backend 'build-pyinstaller-onefile-v8') `
